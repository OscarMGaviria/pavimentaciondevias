...
class ChartsManager {
    constructor(data) {
        ...
        // Avances fijos por subregión (override de datos reales)
        this.AVANCE_SUBREGION = {
            'Oriente': 0.0
            // Todas las demás → 3% (default)
        };
        this.AVANCE_DEFAULT = 3.0; // %
    }

    /* Devuelve el % de avance para una subregión (override fijo) */
    _getAvanceSubregion(nombre) {
        if (nombre && nombre in this.AVANCE_SUBREGION) {
            return this.AVANCE_SUBREGION[nombre];
        }
        return this.AVANCE_DEFAULT;
    }

    /* Sin filtro activo → siempre 3% (valor nominal del proyecto) */
    _getAvanceGlobal() {
        return this.AVANCE_DEFAULT;
    }
    ...
    animateBars(duration) {
        ...
        const animate = function(now) {
            ...
            if (ease < 1) {
                ...
                // Revelar badges de avance una vez que las barras terminaron de crecer
                document.querySelectorAll('.bar-avance-badge').forEach(function(badge) {
    ...
    renderSubregionChart() {
        ...
        allData.forEach(function(item) {
            ...
            const avanceBadge = '<div class="bar-avance-badge"'
                + ' style="position:absolute;top:4px;left:50%;transform:translateX(-50%);'
                + 'font-size:9px;font-weight:700;color:' + avanceColor + ';'
                + 'background:' + badgeBg + ';'
                + 'border:1px solid ' + avanceColor + ';'
            ...
            barsHTML += '<div class="bar-container" data-subregion="' + item.name
                ...
                + '<br><span style="color:#3AF9A2;font-size:10px;">Avance: ' + avancePct.toFixed(1) + '%</span></div>'
                + '<div class="bar" data-target-height="' + height + '" style="width:75%;height:0%;background:' + barBg + ';border-radius:5px 5px 0 0;margin-top:auto;transition:background 0.3s ease,transform 0.22s ease;position:relative;overflow:visible;' + ringStyle + '">'
                + avanceBadge
                + '</div>'
    ...
    renderProgressRow() {
        ...
        const pctAvance = this.selectedSubregion
            ? this._getAvanceSubregion(this.selectedSubregion)
            : this._getAvanceGlobal();

        // Avance en km = mismo valor fijo (no calculado desde km reales)
        const pctKm = pctAvance;

        // Si hay subregión activa, ambas animaciones parten del 100% y retroceden
                ...
                <div class="prow-donut-wrap">
                    <svg class="prow-donut" viewBox="0 0 400 400">
                        ...
                        <circle class="prow-arc"
                            ...
                            stroke-dashoffset="${(CIRC - (fromPct / 100) * CIRC).toFixed(2)}"
                            data-from="${(CIRC - (fromPct / 100) * CIRC).toFixed(2)}"
    ...
    _animateProgress() {
        ...
        const pctText  = document.querySelector('.prow-donut text:first-of-type');
        const CIRC     = arc ? parseFloat(arc.getAttribute('stroke-dasharray')) : 1130.97;

        // Leer from y target desde los data attributes
        const fromOffset  = arc     ? parseFloat(arc.dataset.from)          : CIRC;
        const targetOffset= arc     ? parseFloat(arc.dataset.target)        : CIRC;
        const fromWidth   = barFill ? parseFloat(barFill.dataset.from  || 0): 0;
        const targetWidth = barFill ? parseFloat(barFill.dataset.target)    : 0;

        // % real que corresponde al target del arco (para animar el número)
        const fromPct   = fromWidth;   // mismo valor que data-from de la barra
        const targetPct = targetWidth; // mismo valor que data-target de la barra

        // Si viene del 100% usamos ease-in-out (desaceleración al final)
        // Si viene del 0% usamos ease-out normal
        const easeOut   = t => 1 - Math.pow(1 - t, 3);
        const easeInOut = t => t < 0.5 ? 4*t*t*t : 1 - Math.pow(-2*t+2,3)/2;
        const easeFn    = fromWidth > 0 ? easeInOut : easeOut;
        ...
        const tick = (now) => {
            ...
            // Interpolar arco SVG
            if (arc) {
                arc.style.strokeDashoffset = fromOffset + (targetOffset - fromOffset) * ease;
            }
            // Interpolar barra km
            if (barFill) {
                barFill.style.width = (fromWidth + (targetWidth - fromWidth) * ease) + '%';
            }
            // Animar número del donut (muestra el avance físico, no los km)
            if (pctText) {
